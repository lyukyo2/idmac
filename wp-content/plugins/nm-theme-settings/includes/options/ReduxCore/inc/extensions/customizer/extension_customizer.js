...
(function( $ ) {  //This functions first parameter is named $
    ...
    redux.customizer.inputSave = function( $parent ) {
        ...
        if ($parent.hasClass('redux-container-image_select')) {
            var $nData = $parent.find('.redux-image-select-selected input').attr('value');
        } else {
            var $nData = $parent.find( ':input' ).serializeJSON();
        ...
        if ($nData.length) {
            console.log('NM Customizer - Setting value: '+$nData);
        }
        // /NM
