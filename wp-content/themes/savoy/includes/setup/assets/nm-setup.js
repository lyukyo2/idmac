(function($) {
    ...
    NM_Setup.prototype = {
        ...
        setupNoticeShow( notice, type ) {
            ...
            $notice.children('p').html('安装错误: '+notice);
            $notice.removeClass().addClass('notice notice-'+type);
            
            console.log('NM 安装 - '+notice);
        },
        ...
        pageBuilderSelect: function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    console.log('NM 安装 - 任务完成: 选择页面编辑器');
                    console.log('NM 安装 - 回应: '+response);
                ...
                if (typeof response == 'object' && typeof response.message != 'undefined') {
                    ...
                    if (typeof response.url != 'undefined') {
                        ...
                        if (response.hash == current_item_hash) {
                            $current_node.find('span').text('失败');
                            _findNext();
                        } else {
                            ...
                            jQuery.post(response.url, response, function(response2) {
                                ...
                                $current_node.find('span').text('验证中');
                            }).fail(_ajaxCallback);
                        }
                    ...
                    $current_node.find('span').text('Ajax 错误');
                    _findNext();
                }
            };
        ...
        contentInstall: function() {
            ...
            var _setProgressMessage = function(selector, message, hide) {
                ...
                _ajaxCallback = function(response, taskComplete) {
                    console.log('NM 安装 - 任务完成: '+taskComplete);
                    console.log('NM 安装 - 回应: '+response);
                },
                ...
                _ajaxInstallSettings = function() {
                    $.ajax({
                        ...
                        beforeSend: function() {
                            _setProgressMessage('.nm-setup-task-settings', '配置网站...');
                        },
                        error: function(jqXHR, textStatus, errorThrown) {
                            self.setupNoticeShow('_ajaxInstallSettings(): '+errorThrown, 'error');
                            self.stepHideLoader();
                            _setProgressMessage('.nm-setup-task-settings', '<em class="error">失败, 请重试</em>');
                        },
                        success: function(response) {
                            _setProgressMessage('.nm-setup-task-settings', '完成', true);
                ...
                _ajaxInstallWidgets = function() {
                    $.ajax({
                        ...
                        beforeSend: function() {
                            _setProgressMessage('.nm-setup-task-content', '导入小部件...');
                        },
                        error: function(jqXHR, textStatus, errorThrown) {
                            self.setupNoticeShow('_ajaxInstallWidgets(): '+errorThrown, 'error');
                            self.stepHideLoader();
                            _setProgressMessage('.nm-setup-task-content', '<em class="error">失败, 请重试</em>');
                        },
                        success: function(response) {
                            _setProgressMessage('.nm-setup-task-content', '完成', true);
                ...
                _ajaxInstallContent = function() {
                    $.ajax({
                        ...
                        beforeSend: function() {
                            _setProgressMessage('.nm-setup-task-content', '导入内容...');
                        },
                        error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        success: function(response) {
                            _setProgressMessage('.nm-setup-task-content', '完成');
                ...
                _ajaxInstallWooCommerceTaxonomies = function() {
                    $.ajax({
                        ...
                        beforeSend: function() {
                            _setProgressMessage('.nm-setup-task-content', '安装 WooCommerce 分类法...');
                        },
                        error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        success: function(response) {
                            ...
                            if (response == 'woocommerce na') {
                                console.log('NM 设置 - 错误: WooCommerce 尚未安装，终止安装.');
                                _setProgressMessage('.nm-setup-task-content', '<em class="error">WooCommerce已安装但未启用, 请重试</em>');
